...
export function ProjectPin({
  ...
  // Shared motion + placement, so the two-button card behaves on the board
  // exactly like every single-link card.
  const motionProps = {
    ...
    whileHover: reduced
      ? undefined
      : { scale: 1.02, y: -4, rotate: project.rotate * 0.55, zIndex: 50 },
  };

  // A card with a written case study is not a single destination, so it can't be
  // one link — it carries the title plus a Tech / Case Study choice instead. Every
  // other card stays a whole-card link to its project page.
  if (project.caseStudy) {
    return (
      <motion.div {...motionProps}>
        <CardStock index={index} tag={project.tag}>
          <div className="relative flex flex-1 flex-col items-center justify-center gap-2.5 px-3">
            <span
            ...
            <div className="flex w-full max-w-[86%] gap-1.5">
              <CardButton href={`/projects/${project.id}`} label={`${project.title} — Tech`}>
                Tech
              </CardButton>
              <CardButton href={project.caseStudy} label={`${project.title} — Case study`}>
                Case study
              </CardButton>
            </div>
          </div>
  ...
  return (
    <MotionLink href={`/projects/${project.id}`} aria-label={project.title} {...motionProps}>
      <CardStock index={index} tag={project.tag}>
...
// The printed card stock — grain, ruled grid, crop marks, index and footer — with
// the centre left open. Shared so the single-link and two-button cards are the
...
// A single choice on a two-button card. Bordered and mono to read as a stamped
// marking on the stock, not a web button.
function CardButton({
  href,
  label,
  children,
  ...
  return (
    <Link
      href={href}
      aria-label={label}
      className="flex-1 rounded-[3px] border px-2 py-1.5 text-center font-mono text-[9px] uppercase tracking-[0.14em] outline-none transition-colors hover:bg-[rgba(184,177,165,0.08)] focus-visible:bg-[rgba(184,177,165,0.08)]"
      style={{ borderColor: "rgba(184,177,165,0.28)", color: "#DAD5CA" }}
    >
      {children}
    </Link>
  );
